...
class MangoScraper(BaseScraper):
    def get_product_info(self, url: str) -> dict | None:
            ...
            try:
                ...
                if data.get("@type") in ("Product", "product"):
                    ...
                    if isinstance(image, list):
                        image = image[0]
                    return {
                        "name": data.get("name", "Mango Ürünü"),
        ...
        price_el = soup.find(class_=re.compile("product-sale-price|price"))
        if price_el:
        ...
        img = soup.find("img", class_=re.compile("product.*image|image.*product"))
        if img:
